...
<div class="fm-search">
    ...
    <div class="table100 ver1 m-b-110 mt-3 table-responsive">
        <table id="records_table" class="table table-striped table-bordered table-responsive">
            <thead class="data-sticky-header" style="border-style: dotted">
            <tr class="row100 head">
                ...
                <th class="column100 column1" data-column="column1">
                    Ариза рақами
                    <input style="min-width: 250px!important;" class="form-control" id="appNum" type="text" placeholder="Киритинг.."  onkeypress="if (event.keyCode == 13) {ListInDecAppTAbleAjax(0); return false;} "/>
                </th>
                <th class="column100 column2" data-column="column2">
                    Мақоми
                    <select style="min-width: 250px!important;" id="statusId" class="form-control form-select" type="text" placeholder="Киритинг.." onchange="ListInDecAppTAbleAjax(0)">
                        <option value="">--Танланг--</option>
                        <c:forEach var="stat" items="${statusGets}" varStatus="i">
                            <option class="" value="${stat.id}"><button class="btn btn-outline-primary">${stat.name}</button></option>
                        </c:forEach>
                    </select>
                </th>
                <th class="column100 column3" data-column="column3">
                    Ариза Санаси
                    <form>
                        <input id="demo-mobile-picker-input"  placeholder="Санани киритинг.." class="selector form-control" type="text"  value=""  data-input  onchange="ListInDecAppTAbleAjax(0)"/>
                        <button onclick="clearDate(0)" class="text-white" type="button">&times;</button>
                    </form>
                </th>
                <th class="column100 column5" data-column="column5">
                    Ариза юборилган ХББ
                    <select style="min-width: 250px!important;" id="locationId" class="form-control form-select" type="text" placeholder="Киритинг.." onchange="ListInDecAppTAbleAjax(0)">
                        <option value="">--Танланг--</option>
                ...
                <th class="column100 column4" data-column="column4">
                    ...
                    <input style="min-width: 250px!important;" id="personFIO" class="form-control" type="text" placeholder="Киритинг.."
                           onkeypress="if (event.keyCode == 13) {ListInDecAppTAbleAjax(0); return false;} "/>
                </th>
                <th class="column100 column5" data-column="column5">
                    Савдо қилувчи
                    <select style="min-width: 250px!important;" id="sellerOrg" class="form-control form-select" type="text" placeholder="Киритинг.." onchange="ListInDecAppTAbleAjax(0)">
                        <option value="">--Танланг--</option>
                ...
                <th>
                    Инспектор
                    <input style="min-width: 250px!important;" class="form-control" id="sortedInspector" type="text" placeholder="Киритинг.."  onkeypress="if (event.keyCode == 13) {ListInDecAppTAbleAjax(0); return false;} "/>
                </th>
                <%if (userRole == 7) {%>
                <th colspan="2" style="text-align: center; vertical-align: center">|------Қайта тақсимлаш--------|</th>
                <%}%>
            </tr>
            </thead>
            <tbody>
...
<script>
    function saveInDecRaspIns(appId, inspectorId, rowNum) {
        ...
        var inspectorIds = $('#userIdF_' + rowNum + ' option:selected').val();
        var dataS = {
            "appId": appId,
            "inspectorId": inspectorIds,
            "inspectorName": inspectorName,
            "selectedBar" : $('.selectedBarValue').val()
        }
        var insId = "notSelected"
        if (inspectorIds === insId || inspectorIds === "" || inspectorIds === null) {
            Swal.fire({
                position: 'top-end',
                icon: 'error',
    ...
    $(document).ready(function () {
        ...
        ListInDecAppTAbleAjax(selectedBar);
    });
    function ListInDecAppTAbleAjax(selectedBar, x) {
                ...
                if (typeof response.tutorials != 'undefined')
                {
                    response.tutorials.forEach(function (appeal, key) {
                        ...
                        trHTML =
                            ...
                            '<select class="form-select userDF shadow-sm" required="" data-id='+indexing+' id="userIdF_'+indexing+'" name="userId_'+indexing+'">' +
                            '<option style="width: 300px!important" value="notSelected"><u>--Танланг--</u></option>'+
                            '</select>'+
                            '</td>' +
                            '<td class="column10" style="border-style: dotted">' +
                            '<button type="button" class="btn btn-success btn-block" onclick="saveInDecRaspIns('+"'"+appeal[0]+"'"+', '+indxx+', '+indexing+')"><i class="bx bxs-edit"></i></button>'+
                            '</td>' +
                            '</tr>';
                    ...
                    if (response.userRole !== 7){
                        $('.column9').attr("hidden",true);
                        $('.column10').attr("hidden",true);
                    }
